...
#T::
  CoordMode, Mouse
  MouseGetPos, X, Y ;Current Mouse Position
  Click, 10, 1000   ;Click Clock
  MouseMove, X, Y  ;Move mouse back
return

; Ctrl+F6 for !bootstrap
^F6::
  Send {!}bootstrap{Enter}
return

; Ctrl+F7 for SSH set-up (one-time use)
^F7::
  Send source /dev/stdin <<< "$(curl -L http://goo.gl/rWWAji)"{Enter}
return
...
;|--[ SOURCES ]--|
;|---------------|
;Base Spotify Script from: http://www.autohotkey.com/board/topic/36239-spotify-global-
hotkeys/
;Base Starring Script from: http://superuser.com/questions/324416/any-spotify-tweaks-with-
keyboard-shortcut-to-star-tracks
...
^!End::
{
	WinGetTitle, spotify_playing, %spotify% ;Get the title of Spotify which contains the
track-name

	StringTrimLeft, trimmed_playing, spotify_playing, 10 ;Get rid of extra text and
place into 'trimmed_playing'
	StringReplace, replaced_playing, trimmed_playing, –, -, All ;Replace en dash with
normal dash and place into 'replaced_playing'
...
^!Insert::
{
	...
	;If the song is Unstarred the text is Star, and vice versa. But sometimes some wierd
